<?php
  ...
  function addNewPurchase() {
    ...
    if($con) {
      $query = "SELECT ID FROM suppliers WHERE NAME = '$suppliers_name'";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);

      $query = "INSERT INTO purchases (SUPPLIER_NAME, INVOICE_NUMBER, PURCHASE_DATE, TOTAL_AMOUNT, PAYMENT_STATUS, supplier_id) VALUES('$suppliers_name', $invoice_number, '$invoice_date', $grand_total, '$payment_status', '$row[0]')";
      $result = mysqli_query($con, $query);
      if($result)
        echo "Purchase saved...";
      else
