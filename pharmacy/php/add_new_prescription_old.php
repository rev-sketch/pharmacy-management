<?php
  ...
  if(isset($_GET['action']) && $_GET['action'] == "current_prescription_number")
    getInvoiceNumber();
  ...
  function createMedicineInfoRow() {
    ...
    <div class="row col col-md-12">
      ...
      <div class="col col-md-1"><input type="number" class="form-control" id="mrp_<?php echo $row_number; ?>" onchange="getTotal('<?php echo $row_number; ?>');" ></div>
      <div class="col col-md-1">
  ...
  function getInvoiceNumber() {
    require 'db_connection.php';
    if($con) {
      $query = "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'pharmacy' AND TABLE_NAME = 'prescription';";
      $result = mysqli_query($con, $query);
      $row = mysqli_fetch_array($result);
      echo $row['AUTO_INCREMENT'];
    }
  ...
  function addNewInvoice() {
    ...
    require "db_connection.php";
    if($con) {
      $query1 = "INSERT INTO prescription (prescription_id, customer_id) VALUES ($prescription_id, $customer_id)";
      $query2 = "INSERT INTO prescription_detail (strenght, dose, quantity, medicine_id, prescription_id) VALUES ($strength, $dose, $quantity, $medicine_id, $prescription_id)";
      $query = "INSERT INTO invoices (CUSTOMER_ID, INVOICE_DATE, TOTAL_AMOUNT, TOTAL_DISCOUNT, NET_TOTAL) VALUES($customer_id, '$invoice_date', $total_amount, $total_discount, $net_total)";
      $result = mysqli_query($con, $query);
      ...
      // echo("Error description: " . mysqli_error($con));
      echo ($result) ? "Invoice saved..." : "falied to add invoice...";
      // echo ($result1 && $result2) ? "Prescription saved..." : "failed to add invoice...";
    }
  }
?>