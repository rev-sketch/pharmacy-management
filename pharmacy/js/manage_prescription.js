function deleteInvoice(invoice_number) {
  ...
  if (confirmation) {
    ...
    xhttp.open(
      "GET",
      "php/manage_prescription.php?action=delete&invoice_number=" +
        invoice_number,
      true
    );
    xhttp.send();
...
function refresh() {
  ...
  xhttp.open("GET", "php/manage_prescription.php?action=refresh", true);
  xhttp.send();
}
...
function searchInvoice(text, tag) {
  ...
  xhttp.open(
    "GET",
    "php/manage_prescription.php?action=search&text=" + text + "&tag=" + tag,
    true
  );
  xhttp.send();
}

function searchPrescription(text, tag) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function () {
    if ((xhttp.readyState = 4 && xhttp.status == 200))
      document.getElementById("invoices_div").innerHTML = xhttp.responseText;
...
function printInvoice(invoice_number) {
  ...
  xhttp.open(
    "GET",
    "php/manage_prescription.php?action=print_invoice&invoice_number=" +
      invoice_number,
    false
  );
  xhttp.send();
